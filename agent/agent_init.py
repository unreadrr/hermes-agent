...
def init_agent(
    ...
    # personal (ee3e6d760): when repair_message_sequence rewrites messages
    # in place, set this flag so the next _flush_messages_to_session_db()
    # call replaces the entire transcript (atomic) instead of appending.
    # Without it, repaired sequences leak partial duplicates into SessionDB
    # because append-from-cursor sees the new merged messages as "new".
    agent._session_db_needs_rewrite = False
