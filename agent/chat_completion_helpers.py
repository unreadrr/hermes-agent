...
# personal (b10da72c5): circuit-breaker thresholds for try_activate_fallback.
# Without these protections, a chain of providers all returning
# non-retryable errors back-to-back can produce hundreds of activations
# per second and exhaust host memory on constrained boxes.
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    mappings.
    """
    # personal (b10da72c5): circuit-breaker — throttle consecutive
    # activations and trip the breaker (returning False, which existing
    # call sites already handle as "chain exhausted") if too many fire in
    # a short window.  Without this, when every provider in the chain
    # returns non-retryable errors back-to-back (depleted primary +
    # rate-limited A + 400-rejected B) this function gets called hundreds
    # of times per second on large sessions, exhausting host memory/swap.
    if not hasattr(agent, "_fallback_activations"):
        agent._fallback_activations = []
    now = time.monotonic()
    agent._fallback_activations = [
        t for t in agent._fallback_activations
        if now - t < _FALLBACK_BREAKER_WINDOW_S
    ]
    if len(agent._fallback_activations) >= _FALLBACK_BREAKER_LIMIT:
        agent._emit_status(
            f"🛑 Circuit-breaker tripped: {_FALLBACK_BREAKER_LIMIT} fallback "
            f"activations in {int(_FALLBACK_BREAKER_WINDOW_S)}s. "
            f"Aborting to prevent retry storm."
        )
        return False
    if agent._fallback_activations:
        elapsed = now - agent._fallback_activations[-1]
        if elapsed < _FALLBACK_THROTTLE_INTERVAL_S:
            wait = _FALLBACK_THROTTLE_INTERVAL_S - elapsed
            agent._emit_status(
                f"⏸️ Circuit-breaker: sleeping {wait:.1f}s before fallback switch"
            )
            time.sleep(wait)
    agent._fallback_activations.append(time.monotonic())
