...
def _call_llm_impl(
    ...
    *,
    ...
    # #21566: bump aux-call timeout for local endpoints (Kiro, Ollama, LM Studio).
    # local endpoints can be slow under load; explicit base_url-aware timeout
    # prevents premature ReadTimeouts.
    effective_timeout = (
        timeout
        if timeout is not None
...
async def async_call_llm(
    ...
    *,
    ...
    # api.moonshot.ai vs api.kimi.com/coding even on auto-detected routes.
    _client_base = str(getattr(client, "base_url", "") or "")
    # #21566: bump aux-call timeout for local endpoints.
    effective_timeout = (
        timeout
        if timeout is not None
