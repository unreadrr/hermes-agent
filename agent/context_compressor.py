...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _find_tail_cut_by_tokens(
        ...
        n = len(messages)
        # personal: respect protect_last_n from config (was hardcoded 3,
        # which caused tail to collapse to last 3 messages whenever the
        # token budget walked back to head without hitting the ceiling —
        # very common after _prune_old_tool_results shrinks tool outputs
        # to 1-line summaries before this function runs).
        min_tail = min(self.protect_last_n, n - head_end - 1) if n - head_end > 1 else 0
        ...
        # personal: when the token budget protects everything past the head
        # (loop walked to head_end without hitting soft_ceiling), it means
        # there is no middle region that needs LLM summarisation — phase 1
        # pruning has already done the work.  Returning head_end signals
        # "no middle to compress" and the caller (compress()) short-circuits
        # via `if compress_start >= compress_end: return messages`.
        #
        # The previous behavior (cut_idx = max(fallback_cut, head_end + 1))
        # would FORCE the tail to collapse to the last `min_tail` messages
        # even when the entire conversation fit within budget — losing
        # ~99% of context for no reason.  This was the bug behind sessions
        # going from 1599 messages → 14 with ~10K final tokens out of
        # a 1M-context window.
        if cut_idx <= head_end:
            cut_idx = head_end
