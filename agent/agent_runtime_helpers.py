...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    if repairs > 0:
        ...
        messages[:] = merged
        # personal (ee3e6d760): mark transcript as needing full rewrite —
        # subsequent _flush_messages_to_session_db() will use replace_messages
        # rather than the append-from-cursor path, which would otherwise
        # double-write the merged tail.
        agent._session_db_needs_rewrite = True
